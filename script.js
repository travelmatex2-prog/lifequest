...
async function doRegister(){
  ...
  if(result.success){
    const u={id:result.user_id,username:user,password_hash,pin_hash,xp_total:0,level:1,streak_days:0,last_active:today(),public_profile:true,avatar:'',nations:[],stats:{mente:0,corpo:0,cultura:0,sociale:0,'produttività':0,sfide:0},trophies:[],privacy:{},friends:[],friend_names:{}};
...
function renderFeed(){
  ...
  if(!posts.length){
    el.innerHTML='<div class="empty" style="padding:20px 0"><div class="empty-emoji">🌍</div><div class="empty-text">Nessuna attività nel feed.<br><small style="color:var(--text3)">'+(feedMode==='friends'?'Aggiungi amici o passa a "Tutti"':'Imposta le tue nazioni nel profilo')+'</small></div></div>';
    return;
  }
...
async function toggleQuest(id,e){
  ...
  awardXP(q.xp_base,CAT_STAT[q.category]||'produttività','— '+q.name);
...
function doRoutine(itemId){
  ...
  if(itemId==='custom'){openModal('modal-custom-routine');return;}
...
function saveCustomRoutine(){
  ...
  awardXP(xp,'produttività','— Routine: '+name);
  addFeedPost('Routine: '+name,'produttività',xp,'','');
  checkTrophies();renderRoutine();
}
...
function addRule(type){
  ...
  const titles={metrica:'📊 Metrica',durata:'⏱️ Durata',condizione:'🔁 Condizione',penalita:'⚠️ Penalità'};
...
function addFriend(friendId,friendUsername){
  ...
  if(u.friends.includes(friendId)){showToast('Già amici!');return;}
...
function doLogout(){
  ...
  CUR=null;
  localStorage.removeItem('lq_cur_v5');localStorage.removeItem('lq_cur_v4');localStorage.removeItem('lq_cur_v3');
...
window.addEventListener('load',()=>{
  if(!CUR){
    try{CUR=JSON.parse(localStorage.getItem('lq_cur_v4')||localStorage.getItem('lq_cur_v3')||'null');}catch(e){}
  ...
  if(CUR){
    ...
    if(!u.privacy)u.privacy={};
    if(!u.stats)u.stats={mente:0,corpo:0,cultura:0,sociale:0,'produttività':0,sfide:0};
    saveDB();syncCUR(u);bootApp();
    // Sync silenzioso dal cloud
